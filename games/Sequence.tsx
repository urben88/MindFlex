  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-black transition-colors">
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-[200px]">
        {status === 'showing' && (
          <div className="text-9xl font-black text-slate-800 dark:text-white transition-all transform scale-100 animate-in fade-in zoom-in duration-200">
        ...
        {status === 'input' && (
          <div className="flex flex-col items-center space-y-4">
            <p className="text-slate-400 dark:text-neutral-500 text-sm font-bold uppercase tracking-widest">Tu Turno</p>
            <div className="flex space-x-2 h-12">
              {Array.from({ length: sequence.length }).map((_, i) => (
                <div 
                  key={i} 
                  className={`w-4 h-4 rounded-full transition-colors duration-200 ${
                    i < userSequence.length ? 'bg-primary dark:bg-indigo-400' : 'bg-slate-200 dark:bg-neutral-800'
                  }`}
                />
              ))}
            </div>
            <div className="h-8 text-2xl font-bold text-slate-700 dark:text-slate-200 tracking-widest">
              {userSequence.join(' ')}
            </div>
          </div>
        )}
